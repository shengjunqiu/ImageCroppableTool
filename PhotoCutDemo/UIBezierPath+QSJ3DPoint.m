...
#define contractionFactor 0.6

@implementation UIBezierPath (QSJ3DPoint)
...
- (void)addBezierThroughPoints:(NSArray *)pointArray
{
    ...
    for (int i = 0; i < pointArray.count; i++) {
        ...
        if (i > 0) {
            ...
            previousControlPoint2 = CGPointMake(previousPoint.x - 0.5 * contractionFactor * centerPointDistance * cos(obliqueAngle), previousPoint.y - 0.5 * contractionFactor * centerPointDistance * sin(obliqueAngle));
            controlPoint1 = CGPointMake(previousPoint.x + 0.5 * contractionFactor * centerPointDistance * cos(obliqueAngle), previousPoint.y + 0.5 * contractionFactor * centerPointDistance * sin(obliqueAngle));
        }
